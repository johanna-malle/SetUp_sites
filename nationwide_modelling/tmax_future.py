...
def main(infile_asp, infile_slope, infile_topo, infile_wet,
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=num_work) as executor:
        ...
        with rasterio.open(infile_asp) as src:
            ...
            profile1['tiled'] = True
            profile1['blockxsize'] = 2000  # 4000 must be multiple of 16
...
if __name__ == '__main__':
    ...
    forest_scenario_all = ['summer']
    model_int = ["low", "high", "soil"]
    numbers_sc = [1, 0.5,
                  0]  # [1, 0.5, 0]   # set to 1 if scenario is RCP8.5, 0.5 if RCP4.5 and 0 if scenario is RCP2.6
    num_workers = p
    ...
    for model in model_int:
        ...
        for scenario in numbers_sc:
            print("Model = " + model)
            TSTART_sz = datetime.datetime.now()  # start_szenario = time.time()
            if scenario == 0:
            ...
            temp_loc_overall = Path('/home/clm5_input/CH2018_temp_avgs/cortasmax/')
            out_folder_tmin = Path('/home/malle/micromap_nationwide/micromaps_out/' + model + '/tmax')
            ...
            for date_num in range(0, 1530, 1):
                ...
                out_micromap_map_temp_rast = out_folder_tmin / Path('microtemp_' + sce + '_' +
                                                                    real_date.strftime('%-Y-%m-%d') + '.tif')

                out_micromap_map_temp_nc = out_folder_tmin / Path('microtemp_' + sce + '_' +
                                                                  real_date.strftime('%-Y-%m-%d') + '.nc')
                ...
                if out_micromap_map_temp_nc.is_file():
                    ...
                    searchstring = sce + '_' + real_date.strftime('%-Y-%m-%d') + '.tif'
                    ...
                    # print(f'Done with predictions itself: {TEND3 - TSTART_sz} [HH:MM:SS]')
                    # ----------------------------------------------------------------------------
                    # Write Netcdf:
                    # ----------------------------------------------------------------------------
                    ...
                    ds.attrs['title'] = 'Micromapping Temperature Predictions Switzerland based on CH2018 data'
                    ds.attrs[
                    ...
                    print(
                        f'Done with pred. for {model} sz {sce}, timestep: {real_date}, took: {TEND3a - TSTART_timestep} [HH:MM:SS]')
